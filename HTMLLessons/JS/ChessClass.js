...
class Figure{
    ...
    checkMove(xPos,yPos,xNew,yNew,board){
...
class Pawn extends Figure {
    ...
    checkMove(xPos,yPos,xNew,yNew,board){
        if (xPos==xNew && yPos===yNew) return MoveState.cannot;
        let idx = this.color*2-1;
        let targetCell = this.getCell(xNew,yNew,board);

        if (xNew == xPos + idx){
            // try fight
            if (yNew == yPos+1 || yNew == yPos-1){
        ...
        if (xNew == xPos +idx + idx && yNew == yPos && this._firstMove){
            if (targetCell.figure == undefined){
                let roadCell = board[(xPos+idx)*8,yPos];
...
class ChessCell{
    constructor(row,cell,figure){
        this.row = row;
        this.cell = cell;
        this.figure = figure;
        let div = document.createElement("div");
        div.classList.add("chessCell");
        if ((row+cell) % 2) div.classList.add("blackCell");
        div.id = this.getID();
        PlayerDesk.deskElement.appendChild(div);
        this.setFigure(figure);
    }
    getID(){
        return "cell"+this.row + this.cell;
    }
...
class PlayerDesk{
    constructor(deskId){
        this.desk=[];
        // add horizontal simbols
        for (let i in PlayerDesk.symbolsNumber){
            this.addDivElement(PlayerDesk.symbolsNumber[i],"gripDeskRow")
        }
        this.addDivElement("1","gripDeskColumn");
        this.desk.push(new ChessCell(0,0,new Rook(FigureColor.black)));
        this.desk.push(new ChessCell(0,1,new Horse(FigureColor.black))); 
        this.desk.push(new ChessCell(0,2,new Bishop(FigureColor.black)));
        this.desk.push(new ChessCell(0,3,new Queen(FigureColor.black)));
        this.desk.push(new ChessCell(0,4,new King(FigureColor.black)));
        this.desk.push(new ChessCell(0,5,new Bishop(FigureColor.black)));
        this.desk.push(new ChessCell(0,6,new Horse(FigureColor.black)));                             
        this.desk.push(new ChessCell(0,7,new Rook(FigureColor.black)));
        this.addDivElement("1","gripDeskColumn");
        this.addDivElement("2","gripDeskColumn");
        for (let i=0;i<8;i++) this.desk.push(new ChessCell(1,i,new Pawn(FigureColor.black)));
        this.addDivElement("2","gripDeskColumn");
        for (let i=2;i<6;i++){
            this.addDivElement(i+1,"gripDeskColumn");
            for (let j=0;j<8;j++) this.desk.push(new ChessCell(i,j,undefined));
            this.addDivElement(i+1,"gripDeskColumn");
        }
        this.addDivElement("7","gripDeskColumn");
        for (let i=0;i<8;i++) this.desk.push(new ChessCell(6,i,new Pawn(FigureColor.white)));
        this.addDivElement("7","gripDeskColumn");
        this.addDivElement("8","gripDeskColumn");
        this.desk.push(new ChessCell(7,0,new Rook(FigureColor.white)));
        this.desk.push(new ChessCell(7,1,new Horse(FigureColor.white))); 
        this.desk.push(new ChessCell(7,2,new Bishop(FigureColor.white)));
        this.desk.push(new ChessCell(7,3,new King(FigureColor.white)));
        this.desk.push(new ChessCell(7,4,new Queen(FigureColor.white)));
        this.desk.push(new ChessCell(7,5,new Bishop(FigureColor.white)));
        this.desk.push(new ChessCell(7,6,new Horse(FigureColor.white)));                                            
        this.desk.push(new ChessCell(7,7,new Rook(FigureColor.white)));
        this.addDivElement("8","gripDeskColumn");
        for (let i in PlayerDesk.symbolsNumber){
            this.addDivElement(PlayerDesk.symbolsNumber[i],"gripDeskRow")
        }
    ...
    addDivElement(html,className){
        let gripDeskCell = document.createElement("div");
        gripDeskCell.innerHTML = html;
        gripDeskCell.classList.add(className);
        PlayerDesk.deskElement.appendChild(gripDeskCell);    
    }

    getCell(x,y){
        return this.desk[x*8+y];
    }

    canAnyMove(cellElement){
        for (let i=0;i<8;i++){
            for (let j=0;j<8;j++){
                if (cellElement.figure != undefined){
                    let r = cellElement.figure.checkMove(cellElement.row,cellElement.cell,i,j,this.desk);
    ...
    setHoverForCells(){
        for (let i=0;i<8;i++){
            for (let j=0;j<8;j++){
                let cellElement = this.getCell(i,j);
                let htmlCellElement = cellElement.getCellElement();
                if (this.canAnyMove(cellElement))
                {
                    htmlCellElement.classList.add("chessCellCanMove");
                }else{
                    htmlCellElement.classList.remove("chessCellCanMove");
                }    
    ...
    static deskElement = document.getElementById("chessTable");
    static symbolsNumber = ['','a','b','c','d','e','f','g','h',''];
    static currentPlayer = FigureColor.white;
  
}

globalThis.chessDesk = new PlayerDesk("chessTable");
